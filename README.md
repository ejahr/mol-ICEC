...
- `run_system.py` : script to start the calculation and plots based on the parameters given in `config.py`. Run with `python run_system.py`.
...
- `calc`: functions for calculating and saving cross sections (`cross_section.py`) and spectra (`spectra.py`), based on `icec`. Includes `fit.py` for fitting photoionization cross sections.

## Example H+ LiH
- `HLiH/config.py` : defines the input parameters for H+ LiH
- `HLiH/H-LiH.py` : starts the calculation and plots for H+ and LiH. Uses classes and functions defined in `icec/` and `plotting/`.
